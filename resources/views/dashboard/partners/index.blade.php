...
@section('title', trans('main.partners'))
@section('main_folder', __('main.web'))
@section('sub_folder', __('main.partners'))
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-header">
                    <div class="d-flex justify-between">
                        <div>
                            <h5 class="mb-0">@lang('main.partners')</h5>
                        </div>
                        <div class="my-auto mt-lg-0 mt-4">
                            @can('add partner')
                                <a href="{{ route('admin.partners.create') }}" class="btn btn-primary btn-sm mb-0">+&nbsp;
                                    @lang('main.new_partner')</a>
                            @endcan
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-flush" id="products-list">
                            ...
                            <tbody>
                                @foreach ($partners as $key => $partner)
                                    <tr>
                                        <td class=" text-sm" style="width: 80px">{{ $partners->firstItem() + $key }}</td>
                                        <td>
                                            <div class="d-flex py-1">
                                                <div class="d-flex flex-column justify-content-center">
                                                    <img class="avatar avatar-sm me-3"
                                                        src="{{ $partner->img }}" alt="image">
                                                </div>
                                            </div>
                                        </td>
                                        <td class="text-sm">{{ $partner->name }}</td>
                                        <td class="text-sm">
                                            <div class=""
                                                style=" display: flex; justify-content: center;align-items: center; height:auto">
                                                @can('edit partner')
                                                    <a href="{{ route('admin.partners.edit', $partner->id) }}" class="mx-3"
                                                ...
                                                @can('delete partner')
                                                    <a href="javascript:;" data-bs-toggle="modal"
                                                        data-bs-target="#modal-delete_{{ $partner->id }}">
